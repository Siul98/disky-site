...
try{
 ...
 renderer.domElement.addEventListener('webglcontextlost',event=>{event.preventDefault();lost=true;front.style.visibility='visible';renderer.domElement.style.visibility='hidden';host.classList.remove('has-drive-model')});
 renderer.domElement.addEventListener('webglcontextrestored',()=>{lost=false;if(model){host.classList.add('has-drive-model');front.style.visibility='hidden';renderer.domElement.style.visibility='visible';draw(last,scan,true)}});
 document.addEventListener('visibilitychange',()=>{if(!document.hidden)draw(last,scan,true)});
