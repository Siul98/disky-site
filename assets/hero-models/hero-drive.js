...
try{
 ...
 const g=await new GLTFLoader().loadAsync(new URL('./weave-black-back-logo.glb',import.meta.url).href);model=new THREE.Group();const asset=g.scene;const box=new THREE.Box3().setFromObject(asset),center=box.getCenter(new THREE.Vector3());asset.position.sub(center);model.add(asset);model.rotation.order="ZYX";model.rotation.set(Math.PI/2,0,0);scene.add(model);model.updateMatrixWorld(true);const size=new THREE.Box3().setFromObject(model).getSize(new THREE.Vector3());model.scale.setScalar(1.36/size.x);window.heroDriveInfo={originalSize:size.toArray()};host.classList.add('has-drive-model');front.style.visibility='hidden';draw(last,scan,true);
}catch(e){console.warn('DISKY hero model fallback',e)}
function draw(lift,progress,force=false){last=lift;scan=progress;if(!model||document.hidden)return;const reduced=matchMedia('(prefers-reduced-motion:reduce)').matches;const turn=Math.min(1,lift*2),turnEase=turn*turn*(3-2*turn);model.rotation.y=reduced?0:Math.PI*(1-turnEase);model.rotation.x=Math.PI/2+(reduced?0:Math.sin(Math.min(1,lift*2)*Math.PI)*.12);renderer.domElement.style.opacity=front.style.opacity;renderer.domElement.style.maskImage=front.style.maskImage;renderer.domElement.style.webkitMaskImage=front.style.webkitMaskImage;if(force||renderedLift!==lift){renderer.render(scene,camera);renderedLift=lift}}
