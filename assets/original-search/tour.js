...
async function demoWait(token,ms){let elapsed=0,last=performance.now();while(elapsed<ms){await new Promise(r=>setTimeout(r,100));const now=performance.now();if(token!==turn)return false;if(visible&&!document.hidden)elapsed+=now-last;last=now}return token===turn}
async function demoCompare(token){
 if(!await demoWait(token,2200))return;document.querySelector('.dupcard')?.click();
 if(!await demoWait(token,3500))return;document.querySelector('#dwNext')?.click();
 if(!await demoWait(token,3500))return;document.querySelector('#dwNext')?.click();
}
async function demoIphone(token){
 if(!await demoWait(token,1800))return;
 for(const cell of [...document.querySelectorAll('.ipg-c')].slice(0,6)){cell.click();if(!await demoWait(token,280))return}
 if(!await demoWait(token,1800))return;document.querySelector('#ipmGo')?.click();
}
